...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagchoose_1)
    {
        bitFieldBitLen += choose_1_LEN;
    }
    if(0x01 == aFlag->flagchoose_2)
    {
        bitFieldBitLen += choose_2_LEN;
    }
    if(0x01 == aFlag->flagswitch)
    {
        bitFieldBitLen += switch_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
   ...
   //     GIZWITS_LOG("gizDataPoint2Event Error , Illegal Param\n");
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagchoose_1)
    {
        info->event[info->num] = EVENT_choose_1;
        info->num++;
        dataPoints->valuechoose_1 = gizVarlenDecompressionValue(bitFieldOffset,choose_1_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += choose_1_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagchoose_2)
    {
        info->event[info->num] = EVENT_choose_2;
        info->num++;
        dataPoints->valuechoose_2 = gizVarlenDecompressionValue(bitFieldOffset,choose_2_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += choose_2_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagswitch)
    {
        info->event[info->num] = EVENT_switch;
        info->num++;
        dataPoints->valueswitch = gizVarlenDecompressionValue(bitFieldOffset,switch_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += switch_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
 ...
 //       GIZWITS_LOG("gizCheckReport Error , Illegal Param\n");
    ...
    if(last->valuechoose_1 != cur->valuechoose_1)
    {
 //       GIZWITS_LOG("valuechoose_1 Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagchoose_1 = 1;
        ret = 1;
    }
    if(last->valuechoose_2 != cur->valuechoose_2)
    {
 //       GIZWITS_LOG("valuechoose_2 Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagchoose_2 = 1;
        ret = 1;
    }
    if(last->valueswitch != cur->valueswitch)
    {
  //      GIZWITS_LOG("valueswitch Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagswitch = 1;
        ret = 1;
    }
    if(last->valuepressure != cur->valuepressure)
    {
  //      GIZWITS_LOG("valuepressure Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagpressure = 1;
        ret = 1;
    }
    if(last->valuecard != cur->valuecard)
    {
   //     GIZWITS_LOG("valuecard Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagcard = 1;
        ret = 1;
    }

    if(0 != memcmp((uint8_t *)&last->valueGPRS,(uint8_t *)&cur->valueGPRS,sizeof(last->valueGPRS)))
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
 //           GIZWITS_LOG("valueGPRS Changed");
            gizwitsProtocol.waitReportDatapointFlag.flagGPRS = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
   ...
   //     GIZWITS_LOG("gizDataPoints2ReportData Error , Illegal Param\n");
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagchoose_1)
    {
        gizVarlenCompressValue(bitFieldOffset,choose_1_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuechoose_1);
        bitFieldOffset += choose_1_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagchoose_2)
    {
        gizVarlenCompressValue(bitFieldOffset,choose_2_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuechoose_2);
        bitFieldOffset += choose_2_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagswitch)
    {
        gizVarlenCompressValue(bitFieldOffset,switch_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueswitch);
        bitFieldOffset += switch_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagpressure)
    {
        gizVarlenCompressValue(bitFieldOffset,pressure_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuepressure);
        bitFieldOffset += pressure_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagcard)
    {
        gizVarlenCompressValue(bitFieldOffset,card_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuecard);
        bitFieldOffset += card_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagGPRS)
    {
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)dataPoints->valueGPRS,sizeof(dataPoints->valueGPRS));
        byteFieldOffset += sizeof(dataPoints->valueGPRS);
    }
